    ...
    return (
      <div ref="wrap">
        <div
          ref="realBox"
          style={pos}
          vOn:mousewheel={wheel}
          vOn:mouseenter={enter}
          vOn:mouseleave={leave}
        >
          <table
  ...
  computed: {
    ...
    singleStep() {
      ...
      if (singleHeight % singleStep !== 0) {
        console.warn(
          '当前单步长不是单条数据高度的约数,请及时调整,避免造成滚动错位*&……%&'
        )
      }
      return step
    }
  ...
  methods: {
    handleCellMouseEnter(event) {
      ...
      if (rangeWidth > cellChild.offsetWidth) {
        ...
        const { offsetLeft, arrowOffsetLeft } = this.getOffsetLeft(
          range,
          event,
          rangeWidth
        )
    ...
    getOffsetLeft(range, event, rangeWidth) {
      ...
      const arrowOffsetLeft =
        leftWidth < 0
          ? rangeLeft + event.target.clientWidth / 2
          : rangeWidth / 2
